<?php

$lang = array(
	'__app_vssupport'	=> 'VS Support',
	'menutab__vssupport' => 'VS Support',
	'menutab__vssupport_icon' => 'medkit',
	'menu__vssupport_tickets' => 'Tickets',
	'menu__vssupport_tickets_tickets' => 'Ticket List',
	'module__vssupport_tickets' => 'Tickets',
	'frontnavigation_vssupport' => 'Support',
	'tickets' => 'Tickets',
	'ticket' => 'Ticket',
	'ticket_submit' => 'Submit Ticket',
	'ticket_create' => 'New Ticket',
	'ticket_lock' => 'Lock Ticket',
	'ticket_lock_desc' => 'Locking a ticket restricts conversation to staff member only and prevents reopening by the issuer.',
	'ticket_locked' => 'This Ticket has been locked.',
	'locked_changed_to_0' => 'Ticket has been Unlocked',
	'locked_changed_to_1' => 'Ticket has been Locked',
	'ticket_issuer_deleted' => 'The issuer of this ticket was deleted. Name and notification email will be kept.',
	'ticket_issuer_merged' => 'The issuer of this ticket was merged into another user. The ticket issuer has been adjusted.',
	'ticket_issuer_validated' => 'A user with the same email as the issuer of this ticket has completed registration. This ticket has been merged into that user.',
	'ticket_issuer_email_changed' => 'The ticket issuer changed their email. The notification email has been adjusted.',
	'ticket_issuer_name_changed' => 'The issuer of this thicket changed their display name.',
	'my_tickets' => 'My Tickets',
	'show_all' => 'Show All',
	'message_add' => 'Add Message',
	'ticked_considered_closed_notice' => 'This ticket is considered to be closed, responding will reopen the ticket.',
	'menu__vssupport_tickets_categories' => 'Manage Categories',
	'category' => 'Category',
	'category_add' => 'Add new Category',
	'category_edit' => 'Edit Category',
	'category_changed_to' => 'Category has been changed to',
	'category_delete' => 'Delete Category',
	'category_replacement' => 'Replacement Category',
	'category_replacement_notice' => 'There are tickets using the category you want to delete. Select a different category to transfer those tickets to.',
	'editor__vssupport_CategoryDisclaimer' => 'Per-Category Disclaimer',
	'disclaimer_desc' => 'This disclaimer is shown to the issuer when creating a ticket in this category.',
	'menu__vssupport_tickets_stati' => 'Manage Stati',
	'status' => 'Status',
	'status_add' => 'Add new Status',
	'status_edit' => 'Edit Status',
	'status_delete' => 'Delete Status',
	'status_changed_to' => 'Status has been changed to',
	'status_replacement' => 'Replacement Status',
	'status_replacement_notice' => 'There are tickets using the status you want to delete. Select a different status to transfer those tickets to.',
	'stati' => 'Stati',
	'is_closing_status' => 'Is closing status?',
	'is_closing_status_desc' => "This status causes tickets to be considered 'closed', causing them to not show up in lists by default.",
	'priority' => 'Priority',
	'priority_changed_to' => 'Priority has been changed to',
	'assigned_to' => 'Assigned To',
	'assign_to' => 'Assign To',
	'ticket_assigned_to' => 'Ticket has been assigned to',
	'ticket_unassigned' => 'Ticket assignment has been removed',
	'text' => 'Text',
	'editor__vssupport_TicketText' => 'Ticket Creation',
	'respond_internal' => 'Add as Internal Note',
	'respond_public' => 'Respond',
	'dont_change' => "Don't change",
	'transfer_and_delete' => 'Transfer and Delete',
	'current' => 'Current',
	'users_invoices' => "%s's Invoices",
	'users_purchases' => "%s's Purchases",
	'issuer_name' => 'Issuer Name',
	'issuer_email' => 'Issuer Email',
	'last_update_at' => 'Last Updated',
	'last_update' => 'Last Updated',
	'created' => 'Created',
	'menu__vssupport_tickets_ticketspreferences' => 'My Preferences',
	'default_reply' => 'Default Reply Text',
	'default_reply_desc' => 'This text will be prepared as a reply when you view a ticket on the AdminCP.',
	'all_new_and_unassigned' => 'New and unassigned',
	'all_unresolved' => 'All not Resolved',
	'all_new_and_assigned_to_me' => 'Assigned to me and New',
	'all_unresolved_and_assigned_to_me' => 'Assigned to me and not Resolved',
	'conversation_length' => 'Conversation length',
	'other_tickets_by_this_issuer' => 'Other Tickets by this Issuer',

	'menu__vssupport_tickets_settings' => 'Settings',
	'vssupport_settings' => 'Settings',
	'vssupport_mail_ingres' => 'VSSupport Email Ingress',
	'email_ingres' => 'Email Ingres',
	'email_ingres_only_supports_imap_for_now' => 'Currently only SMTP ingres is supported.',
	'php_imap_not_installed' => 'Your current installation of PHP does not seem to have the <a href="https://www.php.net/manual/en/imap.installation.php">imap extension</a> active. This extension is required to set up IMAP ingest.',
	'smtp_validate_cert' => 'Validate server certificate',
	'smtp_inbox' => 'SMTP Inbox',
	'smtp_delete_processed_mails' => 'Delete successfully ingested emails',
	'imap_connection_failed' => 'IMAP connection failed with the provided credentials: %s',
	'imap_mailbox_not_found' => 'Specified mailbox was not found: %s',
	'core_smtp_impl_bug_error' => 'Unfortunately, there currently is a typo in `system/Email/Outgoing/Smtp.php:221:16` (misspelled "additionalHeaders"). This prevents the whole ingest tracking from working, so unless you fix that manually and set the constant `I_MANUALLY_PATCHED_THE_CORE_SMTP_IMPLEMENTATION` or IVC releases a fix this option is unavailable.',

	'notifications__vssupport_Tickets' => 'Support Tickets',
	'notifications__vssupport_Tickets_ticket_response' => 'New Response',
	'notifications__vssupport_Tickets_ticket_response_desc' => 'Issued whenever there is a new staff member response to a ticket.',
	'notification_ticket_response' => 'New Response to Ticket #%d',
	'mailsub__vssupport_notification_ticket_response' => 'New Response to Ticket #{$ticketId}',
	'emailtpl_notification_ticket_response' => 'Ticket response notification',
	'mail_part__initiator_responded_to_ticket' => '%s responded to your <a href="%s">Ticket #%d</a>:',
	'mail_part__here_is_the_link_to_ticket_if_not_recognized' => "Here is the link to the ticket in case your email client doesn't recognize it above:",
	'mail_part__here_is_the_link_to_ticket' => "Here is the link to the ticket:",
	'mail_part__do_not_respond_to_this_email' => "Do not respond to this email directly, it is autogenerated and the mailbox is not monitored. Your email response will be ignored.",

	'mailsub__vssupport_notification_ticket_created' => 'Ticket #{$ticketId} Created',
	'emailtpl_notification_ticket_created' => 'Ticket created confirmation',
	'mail_part__ticket_created' => 'Hey %s,<br />you have just created <a href="%s">Ticket #%d</a>:',

	'mailsub__vssupport_moderator_digest' => 'Support Ticket Digest',
	'emailtpl_moderator_digest' => 'Moderator digest',
	'mail_part__you_currently_have_n_open_tickets' => 'You currently have {# [1:Ticket that is][?:Tickets that are]} assigned to you and open:',
	'mail_part__ticket_description' => 'Opened: %s - Last reply by: %s @ %s<br />Status: %s - Category: %s - Priority: %s',
	'mail_part__ticket_description_plain' => 'Opened: %s - Last reply by: %s @ %s - Status: %s - Category: %s - Priority: %s',

	'sending_moderator_digest' => 'Sending moderator daily digests...',

	'r__tickets' => 'Can access Tickets?',
	'r__tickets_manage' => 'Can manage Tickets?',
	'r__categories_manage' => 'Can manage Categories?',
	'r__stati_manage' => 'Can manage Stati?',

	'vssupport_pre_convert_notice' => 'This conversion expects a fresh installation of the new support system. Specifically tickets might fail to convert if others are already present, because of collisions in the new hashes. For now hash collisions have to be manually resolved in the database, should they occur.',

	'convert_categories' => 'Convert Categories',
	'_convert_categories' => 'Categories',
	'convert_stati' => 'Convert Stati',
	'_convert_stati' => 'Stati',
	'convert_tickets' => 'Convert Tickets',
	'_convert_tickets' => 'Tickets',
	'convert_messages' => 'Convert Messages',
	'_convert_messages' => 'Messages',
	'convert_actions' => 'Convert Actions',
	'_convert_actions' => 'Actions',

	'vssupport_convert_failed_to_look_up_user' => 'Failed to find a old member in the new database.',

	'ticket_prio_-2_name' => 'Backlog',
	'ticket_prio_-1_name' => 'Below Average',
	'ticket_prio_0_name'  => 'Normal',
	'ticket_prio_1_name'  => 'Important',
	'ticket_prio_2_name'  => 'Urgent',

	'ticket_category_1_name' => 'Category 1',
	'ticket_category_2_name' => 'Category 2',

	'ticket_status_0_name'  => 'New',
	'ticket_status_16_name' => 'In Progress',
	'ticket_status_17_name' => 'Halted',
	'ticket_status_18_name' => 'Resolved',
	'ticket_status_19_name' => "Won't Fix",
);
